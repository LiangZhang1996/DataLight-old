...
def test(model_dir, cnt_round, run_cnt, _dic_traffic_env_conf):
    ...
    if dic_traffic_env_conf["MODEL_NAME"] in dic_traffic_env_conf["LIST_MODEL_NEED_TO_UPDATE"]:
        ...
        dic_traffic_env_conf["IMITATION"] = False
    ...
    try:
        ...
        total_time = 3600 # dic_traffic_env_conf["RUN_COUNTS2"]
        state = env.reset()
        ...
        while not done and step_num < int(total_time / dic_traffic_env_conf["MIN_ACTION_TIME"]):
            ...
            for i in range(dic_traffic_env_conf["NUM_AGENTS"]):
                ...
                if dic_traffic_env_conf["MODEL_NAME"] in ["EfficientPressLight", "CoLight", "MPLight",
                                                          "AdvancedMPLight", "LiteFRAP", "SimpleDQN",
                                                          "MPLightAttention", "Attend", "General"]:
                    one_state = state
                    action_list = agents[i].choose_action(one_state)
                else:
                    one_state = state[i]
