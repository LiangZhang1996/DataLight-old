    ...
    def generate(self):
        ...
        while not done and step_num < int(self.dic_traffic_env_conf["RUN_COUNTS"] /
            ...
            for i in range(self.dic_traffic_env_conf["NUM_AGENTS"]):
                ...
                if self.dic_traffic_env_conf["MODEL_NAME"] in ["PressLight", "CoLight", "MPLight",
                                                               "AdvancedMPLight", "LiteFRAP", "SimpleDQN",
                                                               "MPLightAttention", "Attend","General"]:
                    one_state = state
                    action = self.agents[i].choose_action(one_state)
                    action_list = action
                elif self.dic_traffic_env_conf["MODEL_NAME"] in ["EfficientMaxPressure", "MaxQueue", "Fixedtime"]:
                    one_state = state[i]
                    action = self.agents[i].choose_action(one_state)
